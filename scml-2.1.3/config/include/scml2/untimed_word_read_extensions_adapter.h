 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_UNTIMED_WORD_READ_EXTENSIONS_ADAPTER_H
#define SCML2_UNTIMED_WORD_READ_EXTENSIONS_ADAPTER_H
...
class untimed_word_read_extensions_adapter : public word_adapter_base
{
public:
...
public:
  untimed_word_read_extensions_adapter(C& c, CallbackType cb, const M& mem);
...
template <typename M, typename C>
inline
untimed_word_read_extensions_adapter<M, C>
create_word_read_adapter(C& c,
                         typename untimed_word_read_extensions_adapter<M, C>::CallbackType cb,
                         const M& mem)
{
  return untimed_word_read_extensions_adapter<M, C>(c, cb, mem);
}
  ...
  mCallback(cb)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "untimed_word_read_extensions_adapter only supported for memory objects of size 1, "
