 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_UNTIMED_WRITE_EXTENSIONS_ADAPTER_H
#define SCML2_UNTIMED_WRITE_EXTENSIONS_ADAPTER_H
...
class untimed_write_extensions_adapter
{
public:
...
public:
  untimed_write_extensions_adapter(C& c, CallbackType cb, const M& mem);
...
template <typename M, typename C>
inline
untimed_write_extensions_adapter<M, C>
create_write_adapter(C& c,
                     typename untimed_write_extensions_adapter<M, C>::CallbackType cb,
                     const M& mem)
{
  return untimed_write_extensions_adapter<M, C>(c, cb, mem);
}
  ...
  mCallback(cb)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "untimed_write_extensions_adapter only supported for memory objects of size 1, "
