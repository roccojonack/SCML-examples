...
class word_write_extensions_adapter_tagged : public word_adapter_base
{
public:
...
public:
  word_write_extensions_adapter_tagged(C& c, CallbackType cb, int id, const M& mem);
...
create_word_write_adapter(C& c,
                          ...
                          const M& mem)
{
  return word_write_extensions_adapter_tagged<M, C>(c, cb, id, mem);
}
