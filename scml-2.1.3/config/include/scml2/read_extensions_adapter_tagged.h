 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_READ_EXTENSIONS_ADAPTER_TAGGED_H
#define SCML2_READ_EXTENSIONS_ADAPTER_TAGGED_H
...
class read_extensions_adapter_tagged
{
public:
...
public:
  read_extensions_adapter_tagged(C& c, CallbackType cb, int id, const M& mem);
...
template <typename M, typename C>
inline
read_extensions_adapter_tagged<M, C>
create_read_adapter(C& c,
                    typename read_extensions_adapter_tagged<M, C>::CallbackType cb,
                    int id,
                    const M& mem)
{
  return read_extensions_adapter_tagged<M, C>(c, cb, id, mem);
}
  ...
  mId(id)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "read_extensions_adapter_tagged only supported for memory objects of size 1, "
