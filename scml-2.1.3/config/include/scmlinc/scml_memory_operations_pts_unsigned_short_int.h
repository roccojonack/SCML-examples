...
#ifndef SCML_MEMORY_OPERATIONS_PTS_UNSIGNED_SHORT_INT_H
#define SCML_MEMORY_OPERATIONS_PTS_UNSIGNED_SHORT_INT_H
...
template< > 
class scml_memory_operations< unsigned short int > : 
  public scml_memory_operations_podt_common< unsigned short int >
{
public:
...
private:
  ...
  value_type operator=( const scml_memory_operations & );
  scml_memory_operations( const scml_memory_operations & );

public:
  // virtuals queries for clients for scml_memory_base
  virtual TypeID getType() const { return MEM_TYPE_UINT16; }
  virtual bool isWriteable() const { return true; }
  virtual const char * getDocumentation() const { return "Uint16 Memory";}
  
  // virtual getter setter for instrumentation
  virtual ErrorCode getRegisterValueUint16( uint16 & result,
					    uint64 elementIndex = 0);
  virtual ErrorCode setRegisterValueUint16( uint16 result,
					    uint64 elementIndex = 0);
...
inline scml_memory_operations< unsigned short int >::value_type
scml_memory_operations< unsigned short int >::operator= ( value_type a )
{
  put( a);
  return get();
}
