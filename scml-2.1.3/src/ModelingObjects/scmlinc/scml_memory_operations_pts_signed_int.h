...
#ifndef SCML_MEMORY_OPERATIONS_PTS_SIGNED_INT_H
#define SCML_MEMORY_OPERATIONS_PTS_SIGNED_INT_H
...
template< > 
class scml_memory_operations< signed int > : 
  public scml_memory_operations_podt_common< signed int >
{
public:
...
public:
  ...
  virtual ErrorCode getRegisterValueInt32(signed long & result, uint64 elementIndex = 0);
  virtual ErrorCode getRegisterValueVecInt32(::std::vector< signed long > & result,
                                             uint64 elementIndex = 0,
                                             uint32 elementCount = 1);
#endif  
};
...
inline scml_memory_operations< signed int >::this_reference_type
scml_memory_operations< signed int >::operator= ( value_type a )
{
  put( a);
  return *this;
}
