...
#ifndef SCML_MEMORY_OPERATIONS_PTS_DOUBLE_H
#define SCML_MEMORY_OPERATIONS_PTS_DOUBLE_H
...
template< > 
class scml_memory_operations< double > : 
  public scml_memory_operations_podt_common< double >
{
public:
...
public:
  ...
  virtual TypeID getType() const { return MEM_TYPE_FLOAT64; }
  virtual bool isWriteable() const { return true; }
  virtual const char * getDocumentation() const { return "Float64 Memory";}
  
  // virtual getter setter for instrumentation
  virtual ErrorCode getRegisterValueDouble( double & result,
					   uint64 elementIndex = 0);
  virtual ErrorCode setRegisterValueDouble( double result,
					   uint64 elementIndex = 0);
};


inline scml_memory_operations< double >::this_reference_type
scml_memory_operations< double >::operator= ( value_type a )
{
  put( a);
  return *this;
}
