 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_UNTIMED_READ_ADAPTER_H
#define SCML2_UNTIMED_READ_ADAPTER_H
...
public:
  untimed_read_adapter(C& c, CallbackType cb, const M& mem);
...
template <typename M, typename C>
inline
untimed_read_adapter<M, C>
create_read_adapter(C& c,
                    typename untimed_read_adapter<M, C>::CallbackType cb,
                    const M& mem)
{
  return untimed_read_adapter<M, C>(c, cb, mem);
}
  ...
  mCallback(cb)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "untimed_read_adapter only supported for memory objects of size 1, "
