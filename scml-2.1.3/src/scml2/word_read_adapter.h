 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_WORD_READ_ADAPTER_H
#define SCML2_WORD_READ_ADAPTER_H
...
class word_read_adapter : public word_adapter_base
{
public:
  typedef typename M::data_type data_type;
  typedef bool (C::*CallbackType)(data_type& data, sc_core::sc_time&);

public:
  word_read_adapter(C& c, CallbackType cb, const M& mem);
...
template <typename M, typename C>
inline
word_read_adapter<M, C>
create_word_read_adapter(C& c,
                         typename word_read_adapter<M, C>::CallbackType cb,
                         const M& mem)
{
  return word_read_adapter<M, C>(c, cb, mem);
}
  ...
  mCallback(cb)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "word_read_adapter only supported for memory objects of size 1, "
