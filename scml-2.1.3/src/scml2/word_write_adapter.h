 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_WORD_WRITE_ADAPTER_H
#define SCML2_WORD_WRITE_ADAPTER_H
...
class word_write_adapter : public word_adapter_base
{
public:
...
public:
  word_write_adapter(C& c, CallbackType cb, const M& mem);
...
template <typename M, typename C>
inline
word_write_adapter<M, C>
create_word_write_adapter(C& c,
                          typename word_write_adapter<M, C>::CallbackType cb,
                          const M& mem)
{
  return word_write_adapter<M, C>(c, cb, mem);
}
  ...
  mCallback(cb)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "word_write_adapter only supported for memory objects of size 1, "
