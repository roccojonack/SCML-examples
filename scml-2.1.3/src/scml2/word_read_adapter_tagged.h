 ...
 *                                                                           *
 *****************************************************************************/
#ifndef SCML2_WORD_READ_ADAPTER_TAGGED_H
#define SCML2_WORD_READ_ADAPTER_TAGGED_H
...
class word_read_adapter_tagged : public word_adapter_base
{
public:
...
public:
  word_read_adapter_tagged(C& c, CallbackType cb, int id, const M& mem);
...
template <typename M, typename C>
inline
word_read_adapter_tagged<M, C>
create_word_read_adapter(C& c,
                         typename word_read_adapter_tagged<M, C>::CallbackType cb,
                         int id,
                         const M& mem)
{
  return word_read_adapter_tagged<M, C>(c, cb, id, mem);
}
  ...
  mId(id)
{
  if (mem.get_size() > 1) {
    SCML2_LOG(mem.get_streams().error())
      << "word_read_adapter_tagged only supported for memory objects of size 1, "
